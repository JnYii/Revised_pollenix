# 京粉报：第二份原算法全文保留，第一份公园资料与紧凑界面。
# BEGIN ORIGINAL SECOND ALGORITHM
...
with st.expander("模型方法与局限", expanded=False):
    st.write("综合评分完整采用原第二份代码：逐种季节活跃度与风速影响计算植物丰度，"
        "再计算局部花粉暴露、天气传播、空气滞留、生物活性和PM2.5影响。")
    st.write("蒿属与草本密度由原来的绿地面积函数估算；各植物的峰月、宽度、权重，"
        "以及最终评分和等级阈值均按原文件保留。没有独立的藜科参数。")
